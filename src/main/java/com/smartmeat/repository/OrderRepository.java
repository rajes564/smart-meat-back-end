...
//    @Query("SELECT COALESCE(SUM(o.total), 0) FROM Order o WHERE DATE(o.createdAt) = CURRENT_DATE AND o.status != 'CANCELLED'")
...
//    		BigDecimal todayTotalSales();
    ...
    @Query("""
    		...
    		AND o.status <> 'CANCELLED'
    		""")
    		BigDecimal todayTotalSales(Instant start, Instant end);

//    @Query("SELECT COUNT(o) FROM Order o WHERE DATE(o.createdAt) = CURRENT_DATE AND o.status != 'CANCELLED'")
//    long todayTransactionCount();
    ...
    @Query("""
    		...
    		AND o.status <> 'CANCELLED'
    		""")
    		long todayTransactionCount(Instant start, Instant end);
    
    

    @Query("SELECT COALESCE(SUM(o.total), 0) FROM Order o WHERE EXTRACT(MONTH FROM o.createdAt) = EXTRACT(MONTH FROM CURRENT_DATE) AND o.status != 'CANCELLED'")
